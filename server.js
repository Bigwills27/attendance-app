    ...
    if (hallsCount === 0) {
      await db.collection('halls').insertMany([
        ...
        },
        {
          name: "ENG HALL B",
          polygon: [
            { lat: 6.472700, lng: 3.199000 },
            { lat: 6.472700, lng: 3.199120 },
            { lat: 6.472800, lng: 3.199120 },
            { lat: 6.472800, lng: 3.199000 }
          ]
        },
        {
          name: "SCIENCE HALL",
          polygon: [
            { lat: 6.473000, lng: 3.199200 },
            { lat: 6.473000, lng: 3.199350 },
...
app.post('/api/courses', async (req, res) => {
  try {
    ...
    const existingCourse = await db.collection('Courses').findOne({ code: code.toUpperCase() });
    if (existingCourse) {
...
app.post('/api/attendance/create', async (req, res) => {
  try {
    ...
    if (activeAttendance) {
      return res.status(400).json({ error: 'There is already an active attendance for this course' });
    }
    ...
    const eventId = `${course.code.toLowerCase()}-${now.toISOString().split('T')[0].replace(/-/g, '')}-${Math.floor(Math.random() * 1000)}`;
    ...
    const result = await db.collection('attendances').insertOne(attendanceEvent);
...
app.post('/api/attendance/submit', async (req, res) => {
  try {
    ...
    const existingIPSubmission = attendance.students.find(student => student.ip === ip);
    if (existingIPSubmission) {
      return res.status(400).json({ error: 'This device has already been used for attendance' });
    }

    // Check if matric number is already used
    const existingMatricSubmission = attendance.students.find(student => student.matric === matric);
    if (existingMatricSubmission) {
      return res.status(400).json({ error: 'You have already signed in for this attendance' });
    }
      ...
      if (location.accuracy === 'test' || location.accuracy === 'manual') {
        ...
        return res.status(400).json({ 
          ...
          debug: {
            ...
            distance: this.calculateDistanceToPolygon ? this.calculateDistanceToPolygon([location.lat, location.lng], polygon) : 'unknown'
          }
        });
      }
    }
    ...
    await db.collection('attendances').updateOne(
      { eventId },
      { $push: { students: studentSubmission } }
    );

    res.json({ success: true, message: 'Attendance recorded successfully' });
  } catch (error) {
    console.error('Error submitting attendance:', error);
    res.status(500).json({ error: 'There was an issue recording your attendance. Please try again.' });
  }
});
...
app.get('/api/attendance/all', async (req, res) => {
  try {
    ...
    let attendances = await db.collection('attendances')
      .find(query)
      .sort({ startTime: -1 })
      .toArray();
